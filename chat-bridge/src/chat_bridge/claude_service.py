...
def _mcp_servers() -> dict[str, dict[str, Any]]:
    ...
    return {
        ...
        # Trainer Gateway -- Authorization: Bearer, not X-API-Key. The
        # gateway's own BearerTokenMiddleware checks this header literally
        # (see trainer-mcp-gateway/auth.py); X-API-Key would just be ignored
...
SCOPES: dict[str, list[str]] = {
    ...
    "canary": ["canary"],
    # Standalone too, but for the opposite reason: an unrelated domain
    # (companies/courses/students), not a trust-model mismatch. Folding it
    # into "all" would make "all" mean "every gateway this app has ever
